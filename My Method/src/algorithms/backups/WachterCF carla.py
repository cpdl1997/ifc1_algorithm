...
class WachterCF(BaseCF):
    ...
    def generate_counterfactuals(self, x, features_to_vary, N, learning_rate=0.01, max_iter=100, _lambda=0.01, target =1):
        return self.wachter_recourse(self.model, x)
    
    def wachter_recourse(torch_model, x: np.ndarray, cat_feature_indices: List[int], binary_cat_features: bool, feature_costs: Optional[List[float]],
    lr: float, lambda_param: float, y_target: List[int], n_iter: int, t_max_min: float, norm: int, clamp: bool, loss_type: str) -> np.ndarray:
        """
