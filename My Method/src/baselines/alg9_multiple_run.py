...
import pandas as pd
import numpy as np
from counterfactual_gen import  Algorithm9 as mymethod9
sys.path.insert(0, os.path.join(os.path.dirname(os.path.realpath(__file__)), "../../../"))
import datetime
import random
...
def call_mymethod9_AR(dataset, data, x, denorm_dict, T, index_x, learning_rate, _lambda, max_iterations, gap_between_prints, size_of_dataset, seed, type_of_model, fixed_ft, user_choice_subset, normalization, max_number_of_permutations, user_no=1):
    return mymethod9.main_automate(dataset = dataset,
        data = data,
        x = x,
...
def run(ch_dataset=None, ch_number_of_users=None, type_of_model=None):
    ...
    data = mymethod9.normalize(data, col_name, normalization)
    ...
    subsets_for_fixed_ft_M3 = mymethod9.sublists_till_size_N(all_ft, N=5) #at max five features fixed
    ...
    for i in range(ch_number_of_users):
        ...
        remaining_subsets = mymethod9.sublists(all_ft_copy)
        user_choice_subset = random.choice(remaining_subsets)
        T = random.randint(10,100)
        number_of_features_changed, cost_of_counterfactual, times, comp, chosen_subset = call_mymethod9_AR(dataset, data, x, denorm_dict, T, index_x, learning_rate, _lambda, max_iterations, gap_between_prints, size_of_dataset, seed, type_of_model, fixed_ft, user_choice_subset, normalization, max_number_of_permutations, i+1)
    ...
    if sum(validity_list)==0:
        ...
        logger.info('{}: Average number of times it failed to detect user preference/changed fixed features is {}. ({})'.format(datetime.datetime.now(), violation_list_avg, violation_list))
        ...
        plt.ylabel('Changes')
        plt.xlabel('User')
        plt.savefig(os.path.join(os.path.dirname(os.path.realpath(__file__)), f'./graphs/plot_{exp_name}_changes_{ch_number_of_users}user_alg9.png'))
        plt.plot(x_values, cost_cf, label=f'Cost for each User')
        plt.ylabel('Cost of Counterfactuals')
        plt.xlabel('User')
        plt.savefig(os.path.join(os.path.dirname(os.path.realpath(__file__)), f'./graphs/plot_{exp_name}_cost_{ch_number_of_users}user_alg9.png'))
        plt.close()
