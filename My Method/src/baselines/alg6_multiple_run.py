...
import pandas as pd
import numpy as np
from counterfactual_gen import Algorithm6 as mymethod6
sys.path.insert(0, os.path.join(os.path.dirname(os.path.realpath(__file__)), "../../../"))
import datetime
import random
...
def call_mymethod6_AR(dataset, T, index_x, learning_rate, _lambda, max_iterations, gap_between_prints, size_of_dataset, seed, type_of_model, fixed_ft, user_choice_subset, normalization, max_number_of_permutations):
    return mymethod6.main(dataset = dataset,
         T = T,
         index_x = index_x,
         lr = learning_rate,
         _lambda = _lambda,
...
def run():
    ...
    subsets_for_fixed_ft_M3 = mymethod6.sublists_till_size_N(all_ft, N=5) #at max five features fixed
    ...
    for i in range(ch_number_of_users):
        ...
        remaining_subsets = mymethod6.sublists(all_ft_copy)
        user_choice_subset = random.choice(remaining_subsets)
        T = random.randint(10,100)
        number_of_features_changed, cost_of_counterfactual, times, comp, chosen_subset = call_mymethod6_AR(dataset, T, index_x, learning_rate, _lambda, max_iterations, gap_between_prints, size_of_dataset, seed, type_of_model, fixed_ft, user_choice_subset, normalization, max_number_of_permutations)
