...
import pandas as pd
import numpy as np
from counterfactual_gen import  Algorithm7 as mymethod7
sys.path.insert(0, os.path.join(os.path.dirname(os.path.realpath(__file__)), "../../../"))
import datetime
import random
...
def call_mymethod7_AR(dataset, T, index_x, learning_rate, _lambda, max_iterations, gap_between_prints, size_of_dataset, seed, type_of_model, fixed_ft, user_choice_subset, normalization, max_number_of_permutations):
    return mymethod7.main(dataset = dataset,
         T = T,
         index_x = index_x,
         lr = learning_rate,
         _lambda = _lambda,
...
def run():
    ...
    if dataset_name=='dataset4':
        max_features_fixed = 2

    subsets_for_fixed_ft_M3 = mymethod7.sublists_till_size_N(all_ft, N=max_features_fixed) #at max five features fixed
    ...
    for i in range(ch_number_of_users):
        ...
        remaining_subsets = mymethod7.sublists(all_ft_copy)
        user_choice_subset = random.choice(remaining_subsets)
        T = random.randint(10,100)
        number_of_features_changed, cost_of_counterfactual, times, comp, chosen_subset = call_mymethod7_AR(dataset, T, index_x, learning_rate, _lambda, max_iterations, gap_between_prints, size_of_dataset, seed, type_of_model, fixed_ft, user_choice_subset, normalization, max_number_of_permutations)
        if number_of_features_changed!=-1:
            logger.info('{}: Fixed features {}, User chosen subset {}, Output subset {}, Number of comparisons {}'.format(datetime.datetime.now(), fixed_ft, user_choice_subset, chosen_subset, comp))
