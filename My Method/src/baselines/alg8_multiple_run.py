...
import pandas as pd
import numpy as np
from counterfactual_gen import  Algorithm8 as mymethod8
sys.path.insert(0, os.path.join(os.path.dirname(os.path.realpath(__file__)), "../../../"))
import datetime
import random
...
def call_mymethod8_AR(dataset, data, x, denorm_dict, T, index_x, learning_rate, _lambda, max_iterations, gap_between_prints, size_of_dataset, seed, type_of_model, fixed_ft, user_choice_subset, normalization, max_number_of_permutations):
...
def run(ch_dataset=None, ch_number_of_users=None, type_of_model=None):
    ...
    subsets_for_fixed_ft_M3 = mymethod8.sublists_till_size_N(all_ft, N=5) #at max five features fixed
    ...
    for i in range(ch_number_of_users):
        ...
        remaining_subsets = mymethod8.sublists(all_ft_copy)
        ...
        if number_of_features_changed!=-1:
            ...
            for i in chosen_subset:
                algo_chosen_subset.append(data.columns[i-1])
