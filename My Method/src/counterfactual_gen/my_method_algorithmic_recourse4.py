...
the dowhy and causal-learn packages.

Use environment alg4
...
from algorithms import FourthMethod
...
import logging
log_file_name = "Log_my_method_algorithmic_recourse4.log"
...
def gen_cf(gap_between_prints, ft, data, N, model, lr, max_iter, l, x, model_scm, labels, graph_dot):
    obj = FourthMethod.FourthMethod(data, model, model_scm, labels, graph_dot)
    min_cost = float('inf')
    best_order = []
    ...
    if len(ft)==1: #That is, only single element sent -> No need to permute or find order twice (once to find best_order as below in else condition, and another when finding with the best order)
        ...
        best_counterfactual, cost_ind = obj.generate_counterfactuals(best_counterfactual, features_to_vary = ft, N=N, learning_rate=lr, max_iter=max_iter, _lambda=l, target = 1, gap_between_prints = gap_between_prints)
        min_cost = cost_ind #cost of the current ordering of actions: perm[0] -> perm[1] -> perm[2] ->....
        
    else:
        list_of_permutations = [list(x) for x in list(itertools.permutations(ft))]
        logger.debug("\n")
        ...
        for perm in list_of_permutations:
            ...
            for i in perm:
                ...
                x_cf, cost_ind = obj.generate_counterfactuals(x_cf, features_to_vary = [i], N=N, learning_rate=lr, max_iter=max_iter, _lambda=l, target = 1, gap_between_prints = gap_between_prints)
    ...
    logger.debug("{}: ********************************************************Best counterfactual = {} with cost = {}********************************************************\n\n".format(datetime.datetime.now(), best_counterfactual, min_cost))
    cost = 0
    # x_cf = x.copy()
    # for i in best_order:
    #     x_cf, cost_ind = obj.generate_counterfactuals(x_cf, features_to_vary = [i], N=N, learning_rate=lr, max_iter=max_iter, _lambda=l, target = 1, gap_between_prints = gap_between_prints)
    #     cost += cost_ind #cost of the current ordering of actions: perm[0] -> perm[1] -> perm[2] ->....
    # return x_cf, cost

    return best_counterfactual, min_cost
...
def run(gap_between_prints, dataset, col_name, data, denorm_dict, N, model, lr, l, max_iter, x, model_scm, labels, T, fixed_ft = None, user_choice_subset = None):
    time_taken = 0.0
    ...
    while(len(W_copy)): #do while W is not empty
        ...
        b_hat_counterfactual, _ = gen_cf(gap_between_prints, b_hat, data, N, model, lr, max_iter, l, x, model_scm, labels, graph_dot)
        ...
        for b in W_second_copy:
            ...
            # logger.debug("{}: Current b = {}".format(datetime.datetime.now(), b))
            logger.debug("\n\n{}: ########################################################Generating Counterfactuals for current b {}########################################################".format(datetime.datetime.now(), b))
            b_counterfactual, _ = gen_cf(gap_between_prints, b, data, N, model, lr, max_iter, l, x, model_scm, labels, graph_dot)

            # logger.debug("{}: x = {}".format(datetime.datetime.now(), x))
            # logger.debug("{}: b_hat_counterfactual = {}".format(datetime.datetime.now(), b_hat_counterfactual))
            # logger.debug("{}: b_counterfactual = {}".format(datetime.datetime.now(), b_counterfactual))
            
            
            #used for printing purposes -> This is for changes reflected overall including causal effects
            ...
            st_hat = ""
            st = ""
            # logger.debug("{}: op_lst_hat = {}".format(datetime.datetime.now(), op_lst_hat))
            # logger.debug("{}: op_lst = {}".format(datetime.datetime.now(), op_lst))
            # print(data.columns)
            for i, val in enumerate(op_lst_hat):
                if val>0:
                    st_hat = st_hat + data.columns[i] + " increased by " + str(abs(val)) + ", " #Just a way to write the change in words
                elif val<0:
            ...
            st_act_ft_hat = ""
            st_act_ft = ""
            for _, val in enumerate(b_hat):
                # logger.debug("{}: val = {}".format(datetime.datetime.now(), val))
                # logger.debug("{}: op_lst_hat[val-1] = {}".format(datetime.datetime.now(), op_lst_hat[val-1]))
                if op_lst_hat[val-1]>0:
                    st_act_ft_hat = st_act_ft_hat + data.columns[val-1] + " increased by " + str(abs(op_lst_hat[val-1])) + ", "
                elif op_lst_hat[val-1]<0:
                    st_act_ft_hat = st_act_ft_hat + data.columns[val-1] + " decreased by " + str(abs(op_lst_hat[val-1])) + ", "
                elif op_lst_hat[val-1]==0:
                    st_act_ft_hat = st_act_ft_hat + data.columns[val-1] + " unchanged" + ", "
            for _, val in enumerate(b):
                # logger.debug("{}: val = {}".format(datetime.datetime.now(), val))
                # logger.debug("{}: op_lst[val-1] = {}".format(datetime.datetime.now(), op_lst[val-1]))
                if op_lst[val-1]>0:
                    st_act_ft = st_act_ft + data.columns[val-1] + " increased by " + str(abs(op_lst[val-1])) + ", "
                elif op_lst[val-1]<0:
                    st_act_ft = st_act_ft + data.columns[val-1] + " decreased by " + str(abs(op_lst[val-1])) + ", "
                elif op_lst[val-1]==0:
                    st_act_ft = st_act_ft + data.columns[val-1] + " unchanged" + ", "
            st_act_ft_hat = st_act_ft_hat.rstrip(', ')
            st_act_ft = st_act_ft.rstrip(', ')
            ...
            if user_choice_subset is None: #When we want to make an interactive input about best subset
                ...
                logger.debug("{}: Option 1: {} with {} changes (Effects: {}).".format(datetime.datetime.now(), st_act_ft_hat, find_change_count(x, b_hat_counterfactual), st_hat))
                logger.debug("{}: Option 2: {} with {} changes (Effects: {}).".format(datetime.datetime.now(), st_act_ft,  find_change_count(x, b_counterfactual), st))
    ...
    final_counterfactual, cost = gen_cf(gap_between_prints, final_candidate, data, N, model, lr, max_iter, l, x, model_scm, labels, graph_dot)
    ...
    final_st_act = ""
    for _, val in enumerate(final_candidate):
        if op_diff[val-1]>0:
            final_st_act = final_st_act + data.columns[val-1] + " increased by " + str(abs(op_diff[val-1])) + ", "
        elif op_diff[val-1]<0:
            final_st_act = final_st_act + data.columns[val-1] + " decreased by " + str(abs(op_diff[val-1])) + ", "
        elif op_diff[val-1]==0:
            final_st_act = final_st_act + data.columns[val-1] + " unchanged" + ", "
    final_st_act = final_st_act.rstrip(', ')
    ...
    return final_counterfactual, cost, T_hat, time_taken, final_candidate
...
def main(dataset, T=100, index_x=-1,  lr=0.01, l=0.1, max_iter=100, gap_between_prints=80, size_of_dataset=5000, seed=1, type_of_model = -1, fixed_ft=None, user_choice_subset=None):
    ...
    if(dataset_name[:-1]=="dataset" and int(dataset_name[-1])<4): #dataset 1-3 are not causal
        ...
        data_copy = data.copy().drop([col_name], axis=1)
        labels = [f'{col}' for _, col in enumerate(data_copy.columns)]
    ...
    #Output section
    best_counterfactual, cost, T_hat, time_taken, final_candidate = run(gap_between_prints, dataset, col_name, data, denorm_dict, N, model,  lr, l, max_iter, x, model_scm, labels, T, fixed_ft, user_choice_subset)
...
if __name__=="__main__":
    ...
    index_x = 0 #1 for HELOC, 2 for dataset3, 3 for dataset4
    learning_rate = 1e-3
    _lambda = 0.7
    max_iterations = 5
