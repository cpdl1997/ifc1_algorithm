...
import logging
log_file_name = "Log_manan_algorithmic_recourse2.log"
...
def gen_cf(gap_between_prints, ft, data, N, model, lr, max_iter, l, x):
    obj = WachterCF.WachterCF2(data, model)
...
def run(gap_between_prints, dataset, order_user, order_user_num, data, N, model, lr, l, max_iter, x):
    ...
    for i in range(N-1):
        ...
        time_start = time()
        x_counterfactual = gen_cf(gap_between_prints, eligible_set_num, data, N, model, lr, max_iter, l, x)
        time_end = time()
        ...
        if x_counterfactual is not None:
            x_counterfactual = x_counterfactual.detach().numpy()
