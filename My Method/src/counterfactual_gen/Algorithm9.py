...
from algorithms import NinthMethod
...
import logging
log_file_name = "Log_algorithm9.log"
...
def gen_cf(gap_between_prints, ft, data, N, model, lr, max_iter, _lambda, x, model_scm, labels, graph_dot, max_number_of_permutations, history_of_causal_effect):
    obj = NinthMethod.NinthMethod(data, model, model_scm, labels, graph_dot, history_of_causal_effect)
    min_cost = float('inf')
...
'''
def run(gap_between_prints, dataset, col_name, data, denorm_dict, N, model, lr, _lambda, max_iter, x, model_scm, labels, T, fixed_ft = None, user_choice_subset = None, normalization = "minmax", max_number_of_permutations = 3, user_no=1):
    time_taken = 0.0
    ...
    #     # logger.debug("{}: subset = {}".format(datetime.datetime.now(), subset))
    #     for i in fixed_ft: #The current subset has a feature which cannot be modified
    #         # logger.debug("{}: i = {} in subset {} which is in W? => {}".format(datetime.datetime.now(), i, subset, i in subset and subset in W))
    ...
    for i, subset in enumerate(W):
        ...
        subset_factor = 1
        dict_of_subset[tuple(subset)] = (i, find_bradley_value(b_hat_factor, subset_factor), np.sqrt(np.log(1/delta)), 1) #(subset ID, P(b_hat>b), Confidence Interval, number of comparisons made for this pair)
        ...
        if b_hat_counterfactual is None: #If we failed to generate counterfactual, we look for another b_hat
            ...
            for b in W_second_copy:
                ...
                if b_counterfactual is None: #If we failed to generate counterfactual, we look for next b
                    ...
                    list_mincost_b_hat.append(b_hat_mincost)#append for graph after all rounds of comparisons are over
                    list_yhat_b_hat.append(b_hat_yhat)
                    list_T.append(T_hat)
    ...
    if final_candidate is not None:
        ...
        if final_counterfactual is None:
            ...
            plt.savefig(os.path.join(os.path.dirname(os.path.realpath(__file__)), f'../baselines/graphs/plot_cost_bhat_vs_comparisons_User{user_no}.png'))
            plt.clf() #clear the figure for the next graph
            plt.plot(list_T, list_yhat_b_hat, label=f'Y_hat v/s comparisons')
            plt.ylabel('Y_hat')
            plt.xlabel('Comparisons')
            plt.savefig(os.path.join(os.path.dirname(os.path.realpath(__file__)), f'../baselines/graphs/plot_y_hat_vs_comparisons{user_no}.png'))
            plt.close()
...
def main_automate(data, dataset, x, denorm_dict, T=100, index_x=-1,  lr=0.01, _lambda=0.1, max_iter=100, gap_between_prints=80, size_of_dataset=5000, seed=1, type_of_model = -1, fixed_ft=None, user_choice_subset=None, normalization = "minmax", max_number_of_permutations = 3, user_no=1):
    #Input section
    if data is None:
        data=pd.read_csv(dataset)
        data = data.dropna()
