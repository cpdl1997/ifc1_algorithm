...
from algorithms import TenthMethod
...
def find_bradley_value(num, denom):
    return np.exp(num)/(np.exp(num) + np.exp(denom))
...
def find_distance(x, x_counterfactual, perm):
...
def gen_cf(gap_between_prints, ft, data, N, model, lr, max_iter, _lambda, x, model_scm, labels, graph_dot, max_number_of_permutations, history_of_causal_effect):
    obj = TenthMethod.TenthMethod(data, model, model_scm, labels, graph_dot, history_of_causal_effect)
    min_cost = float('inf')
    ...
    if len(ft)==1: #That is, only single element sent -> No need to permute or find order twice (once to find best_order as below in else condition, and another when finding with the best order)
        ...
        best_counterfactual, min_cost, y_hat_best_counterfactual = obj.generate_counterfactuals(x.copy(), best_counterfactual, features_to_vary = ft, N=N, learning_rate=lr, max_iter=max_iter, _lambda=_lambda, target = 1, gap_between_prints = gap_between_prints, perturbation_val=perturbation_val/len(ft))
        if best_counterfactual is not None:
            min_cost += _lambda*find_distance(x, best_counterfactual, ft)
        ...
        for perm in list_of_permutations:
            ...
            for i in perm:
                ...
                x_cf, cost_ind, y_hat = obj.generate_counterfactuals(x.copy(), x_cf, features_to_vary = [i], N=N, learning_rate=lr, max_iter=max_iter, _lambda=_lambda, target = 1, gap_between_prints = gap_between_prints, perturbation_val=perturbation_val/len(ft))
            ...
            if x_cf is not None: #A valid counterfactual has been generated
                cost += _lambda*find_distance(x, x_cf, perm) #Add the L1 loss at this instance - if cost_ind had L1 loss it would be finding L1 loss for perm[1], perm[2], ... wrt itself, not x
...
''' Given: 
        ...
        max_number_of_permutations: Maximum number of permutations considered for any intervention state
        user_no: Used for automation with multiple users - gives current user number
...
'''
def run(gap_between_prints, dataset, col_name, data, denorm_dict, N, model, lr, _lambda, max_iter, x, model_scm, labels, T, fixed_ft = None, user_choice_subset = None, normalization = "minmax", max_number_of_permutations = 3, user_no=1, print_graph=1):
    time_taken = 0.0
    ...
    sblist = [] 
    for i in range(1, N):
        if data.columns[i-1] not in fixed_ft: #do not include fixed features
            sblist.append(i)
    ...
    for i, subset in enumerate(W):
        ...
        subset_factor = 1
        dict_of_subset[tuple(subset)] = (i, find_bradley_value(b_hat_factor, subset_factor), np.sqrt(np.log(1/delta)), 1, 0) #(subset ID, P(b_hat>b), Confidence Interval, number of comparisons made for this pair)
    ...
    #store counterfactuals already generated in order to get more optimised results
    counterfactual_dict = {}
    
    user_choice_factor = 0.85 #Reduce the cost by this factor if user chooses that subset
    ...
    while(len(W_copy)): #do while W is not empty
        ...
        if tuple(b_hat) not in counterfactual_dict:
            b_hat_counterfactual, b_hat_mincost, b_hat_yhat = gen_cf(
            ...
            counterfactual_dict[tuple(b_hat)] = (b_hat_counterfactual, b_hat_mincost, b_hat_yhat)
        else:
            b_hat_counterfactual, b_hat_mincost, b_hat_yhat = counterfactual_dict[tuple(b_hat)]
        ...
        if b_hat_counterfactual is None: #If we failed to generate counterfactual, we look for another b_hat
            ...
            for i, subset in enumerate(W_copy): #Recompute P and C for the new b_hat
                ...
                subset_factor = 1
                dict_of_subset[tuple(subset)] = (i, find_bradley_value(b_hat_factor, subset_factor), np.sqrt(np.log(1/delta)), 1) #new tuple since tuples are immutable
            ...
            for b in W_second_copy:
                ...
                if tuple(b) not in counterfactual_dict:
                    b_counterfactual, b_mincost, b_yhat = gen_cf(
                                                gap_between_prints = gap_between_prints,
                                                ft = b,
                                                data = data,
                                                N = N,
                    ...
                    counterfactual_dict[tuple(b)] = (b_counterfactual, b_mincost, b_yhat)
                else:
                    b_counterfactual, b_mincost, b_yhat = counterfactual_dict[tuple(b)]
                ...
                if b_counterfactual is None: #If we failed to generate counterfactual, we look for next b
                    ...
                    if user_choice_subset is None: #When we want to make an interactive input about best subset
                        ...
                        score1 = 0
                        score2 = 0
                        b_hat_ft = [data.columns[t-1] for t in b_hat]
                        b_ft = [data.columns[t-1] for t in b]
                        ...
                        logger.debug("{}: user_choice_subset = {}, b_hat_ft={}, b_ft={}.".format(datetime.datetime.now(), user_choice_subset, b_hat_ft, b_ft))
                    ...
                    if dict_of_subset[tuple(b)][1] >=0.5: #P(b_hat>b) >=0.5
                        if ch == 1: #User agrees with our prediction
                            ...
                            #We reduce the cost of generating b_hat since it is preferred by the user hence it's costs must be less
                            bcf, bmc, by = counterfactual_dict[tuple(b_hat)] #Remember: The tuple is (counterfactual, cost, predicted y)
                            reduction_factor = user_choice_factor*(1-0.3*by) #We reduce on the basis of whether user chose the subset as well as how far we are from being classified correctly
                            counterfactual_dict[tuple(b_hat)] = (bcf, bmc*reduction_factor, by) #cost reduced depending on whether this got chosen
                            ...
                            #We reduce the cost of generating b since it is preferred by the user hence it's costs must be less
                            bcf, bmc, by = counterfactual_dict[tuple(b)] #Remember: The tuple is (counterfactual, cost, predicted y)
                            reduction_factor = user_choice_factor*(1-0.3*by) #We reduce on the basis of whether user chose the subset as well as how far we are from being classified correctly
                            counterfactual_dict[tuple(b)] = (bcf, bmc*reduction_factor, by) #cost reduced depending on whether this got chosen
                    ...
                    list_mincost_b_hat.append(counterfactual_dict[tuple(b_hat)][1])#append for graph after all rounds of comparisons are over
                    list_yhat_b_hat.append(counterfactual_dict[tuple(b_hat)][2])
                    list_T.append(T_hat)
            ...
            if set(next_candidate) != set(b_hat):
                ...
                for i, subset in enumerate(W_copy): #Recompute P and C for the new b_hat
                    # dict_of_subset[tuple(subset)] = (i, np.exp(-len(b_hat))/(np.exp(-len(subset)) + np.exp(-len(b_hat))), np.sqrt(np.log(1/delta)), 1)
                    subset_factor = 1
                    dict_of_subset[tuple(subset)] = (i, find_bradley_value(b_hat_factor, subset_factor), np.sqrt(np.log(1/delta)), 1)
            else:
    ...
    if final_candidate is not None:
        ...
        if tuple(final_candidate) not in counterfactual_dict:
            ...
            counterfactual_dict[tuple(final_candidate)] = (final_counterfactual, cost, y_hat)
        else:
            final_counterfactual, cost, y_hat = counterfactual_dict[tuple(final_candidate)] #We would expect this part to be executing exclusively, otherwise something is wrong LOL
        ...
        if final_counterfactual is None:
            ...
            if print_graph==1: #print only if user asks to
                #create graphs showing change in cost of b_hat and y values with number of comparisons
                folder_name = datetime.datetime.now().strftime(f"{exp_name}_%B%d%Y_%I%M%p_User{user_no}")
                ...
                plt.plot(list_T, list_mincost_b_hat, label=f'Cost of bhat v/s comparisons')
                plt.ylabel('Cost')
                plt.xlabel('Comparisons')
                filename1=os.path.join(os.path.dirname(os.path.realpath(__file__)), f'../../results/graphs/{folder_name}/cost_bhat_vs_comparisons_User{user_no}_{exp_name}.png')
                if os.path.isfile(filename1):
                    os.remove(filename1)
                plt.savefig(filename1)
                plt.clf() #clear the figure for the next graph
                plt.plot(list_T, list_yhat_b_hat, label=f'Y_hat v/s comparisons')
                plt.ylabel('Y_hat')
                plt.xlabel('Comparisons')
                filename2=os.path.join(os.path.dirname(os.path.realpath(__file__)), f'../../results/graphs/{folder_name}/y_hat_vs_comparisons_User{user_no}_{exp_name}.png')
        ...
        # list_graph = []
        # list_graph.append((list_mincost_b_hat, list_yhat_b_hat, list_T))
    else:
        logger.debug("{}: No solution was found!".format(datetime.datetime.now()))
        final_counterfactual = None
        cost = -1
    ...
    return final_counterfactual, cost, T_hat, time_taken, final_candidate, y_hat, (list_mincost_b_hat, list_yhat_b_hat, list_T)
...
'''
def main(dataset, T=100, index_x=-1,  lr=0.01, _lambda=0.1, max_iter=100, gap_between_prints=80, size_of_dataset=5000, seed=1, type_of_model = -1, fixed_ft=None, user_choice_subset=None, normalization = "minmax", max_number_of_permutations = 3, print_graph=1):
    #Input section
    data=pd.read_csv(dataset)
    data = data.dropna()
    if(size_of_dataset>data.shape[0]):
        size_of_dataset = data.shape[0]
    data = data.head(size_of_dataset).reset_index(drop=True) #size of dataset to be used
    ...
    if(dataset_name[:-1]=="dataset" and int(dataset_name[-1])<4): #dataset 1-3 are not causal
        ...
        logger.debug('{}: Creating Causal Model....'.format(datetime.datetime.now()))
        ...
        logger.debug('{}: Causal Model created.'.format(datetime.datetime.now()))
    ...
    #Output section
    best_counterfactual, cost, T_hat, time_taken, final_candidate, y_hat, tup_graph = run(
                                                                        gap_between_prints = gap_between_prints,
                                                                        dataset = dataset,
                                                                        col_name = col_name,
                                                                        data = data,
    ...
    if(best_counterfactual is not None and y_hat>0.5):
        ...
        algo_chosen_subset=[]
        for t in final_candidate:
            algo_chosen_subset.append(data.columns[t-1])
        logger.info("-------------------------------------------------------------------------------------------------------\n")
        logger.info('{}: Original data point is {}'.format(datetime.datetime.now(), x))
        logger.info('{}: The counterfactual is given by {} with number of features changed = {}, and cost = {}'.format(datetime.datetime.now(), best_counterfactual, number_of_features_changed, cost_of_counterfactual))
        logger.info('{}: Subset chosen by algorithm is {}, whereas actual subset chosen by user is {}. (Fixed features = {})'.format(datetime.datetime.now(), algo_chosen_subset, user_choice_subset, fixed_ft))
    ...
    return number_of_features_changed, cost_of_counterfactual, time_taken, T_hat, final_candidate, tup_graph



def main_automate(data, dataset, x, denorm_dict, T=100, index_x=-1,  lr=0.01, _lambda=0.1, max_iter=100, gap_between_prints=80, size_of_dataset=5000, seed=1, type_of_model = -1, fixed_ft=None, user_choice_subset=None, normalization = "minmax", max_number_of_permutations = 3, user_no=1, print_graph=1):
    #Input section
    if data is None:
        data=pd.read_csv(dataset)
        data = data.dropna()
