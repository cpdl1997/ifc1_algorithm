...
from algorithms import EightMethod
...
import logging
log_file_name = "Log_algorithm8.log"
...
def gen_cf(gap_between_prints, ft, data, N, model, lr, max_iter, _lambda, x, model_scm, labels, graph_dot, max_number_of_permutations, history_of_causal_effect):
    # obj = SixthMethod.SixthMethod(data, model, model_scm, labels, graph_dot, history_of_causal_effect)
    obj = EightMethod.EightMethod(data, model, model_scm, labels, graph_dot, history_of_causal_effect) #This works the best
    ...
    if len(ft)==1: #That is, only single element sent -> No need to permute or find order twice (once to find best_order as below in else condition, and another when finding with the best order)
        ...
        # list_of_permutations = random.sample(list_of_permutations, max_number_of_permutations)
...
def main_automate(data, dataset, x, denorm_dict, T=100, index_x=-1,  lr=0.01, _lambda=0.1, max_iter=100, gap_between_prints=80, size_of_dataset=5000, seed=1, type_of_model = -1, fixed_ft=None, user_choice_subset=None, normalization = "minmax", max_number_of_permutations = 3):
    #Input section
    if data is None:
        data=pd.read_csv(dataset)
        data = data.dropna()
